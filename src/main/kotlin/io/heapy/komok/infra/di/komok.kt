...
suspend inline fun <reified T : EntryPoint<R>, R> komok(
    ...
    val source = builder::class.toString()
    val komokBuilder = DefaultKomokBuilder(source)
    builder(komokBuilder)
    val komok = komokBuilder.komok()
    return komok.run(source, genericKey<T>())
}
 ...
 */
@PublishedApi
internal class DefaultKomokBuilder(
        ...
        val komokModuleProvider = object : ModuleProvider {
            override fun module(): Module {
                return DefaultModule(
                    source = source,
                    dependencies = modules + komokModule,
                    bindings = bindings,
                )
            }
        }
