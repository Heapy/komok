...
import io.heapy.komok.wip.HLCDecodeError.CounterDecodeFailure
import io.heapy.komok.wip.HLCDecodeError.NodeDecodeFailure
import io.heapy.komok.wip.HLCDecodeError.TimestampDecodeFailure
import io.heapy.komok.wip.HLCError.CausalityOverflowError
import io.heapy.komok.wip.HLCError.ClockDriftError
import io.heapy.komok.wip.HLCError.DuplicateNodeError
...
data class HybridLogicalClock(
    ...
    companion object {
        ...
        fun remoteTock(
            ...
            val clock = when {
                local.node.identifier == remote.node.identifier -> {
                    throw DuplicateNodeError(local.node)
                }
        ...
        fun decodeFromString(encoded: String): HybridLogicalClock {
            ...
            if (parts.size < 3) throw TimestampDecodeFailure(encoded)
            ...
            val timestamp = parts
                ...
                ?: throw TimestampDecodeFailure(encoded)

            val counter = parts
                .getOrNull(1)
                ?.toInt(36)
                ?: throw CounterDecodeFailure(encoded)

            val node = parts
                .getOrNull(2)
                ?.let { NodeID(it) }
                ?: throw NodeDecodeFailure(encoded)
