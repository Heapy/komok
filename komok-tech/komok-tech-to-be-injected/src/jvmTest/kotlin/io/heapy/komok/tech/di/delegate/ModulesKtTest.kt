...
private class ReflectiveCycleA(
    val b: ReflectiveCycleB,
)

private class ReflectiveCycleB(
    val a: ReflectiveCycleA,
)
...
class ModulesKtTest {
    ...
    @Test
    fun `reflective dependency cycle is reported`() {
        val exception = assertThrows(IllegalStateException::class.java) {
            val _ = buildModules<ReflectiveCycleA>()
        }

        assertTrue(exception.message!!.contains("Module dependency cycle"))
    }
}
